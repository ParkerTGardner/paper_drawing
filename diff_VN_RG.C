...
double VOut(TH2D* h1, TH2D* h2, int YPlo, int YPhi,int vn)
{
    ...
    func1.SetParameter(6, 0.1);
    func1.SetParameter(7, 0.1);
...
void diff_VN_RG()
{
    TFile *f_reco_co = new TFile("newest_no_co/DCA_no_co_all_data.root");
    TFile *f_data_co   = new TFile("data_co_good.root");
    ...
    const int   trackbinbounds[trackbin]         = { 0,20,30,40,50,60,70,78,86};
    const int   trackbinboundsUpper[trackbin]    = {20,30,40,50,60,70,78,86,1000};


    const float ptbinbounds_lo[ptbin]       = {10};
    ...
    for(int wtrk = 0; wtrk < trackbin; wtrk++){
        ...
        TH2D* h1;
        h1 = (TH2D*)f_reco_co->Get(Form("hSigS_%d_to_%d_and_%d_to_%d",trackbinbounds[wtrk], trackbinboundsUpper[wtrk], pt_lo, pt_hi))->Clone();
        h1->Scale(1/(hJ1_f1_reco_co->GetBinContent(wtrk+1)));
        h1->Scale(1./(BW1));

        TH2D* h2;
        h2 = (TH2D*)f_reco_co->Get(Form("hBckS_%d_to_%d_and_%d_to_%d",trackbinbounds[wtrk], trackbinboundsUpper[wtrk], pt_lo, pt_hi))->Clone();
    ...
    for(int wtrk = 0; wtrk < trackbin; wtrk++){
        TH2D* h1;
        h1 = (TH2D*)f_data_co->Get(Form("hSigS_%d_to_%d_and_%d_to_%d",trackbinbounds[wtrk], trackbinboundsUpper[wtrk], pt_lo, pt_hi))->Clone();
        h1->Scale(1/(hJ1_f2_data_co->GetBinContent(wtrk+1)));
        h1->Scale(1./(BW1));

        TH2D* h2;
        h2 = (TH2D*)f_data_co->Get(Form("hBckS_%d_to_%d_and_%d_to_%d",trackbinbounds[wtrk], trackbinboundsUpper[wtrk], pt_lo, pt_hi))->Clone();
    ...
    TFile* fS_tempA = new TFile("Diff_RG_vs_noRG.root", "recreate");
