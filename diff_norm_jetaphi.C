...
void diff_norm_jetaphi()
{
	//TFile *f_reco_co = new TFile("sys_final_jetEP.root");
	TFile *f_reco_co = new TFile("wtfisgoingon.root");
	TFile *f_data_co   = new TFile("final_final.root");
	//TFile *f_reco_co   = new TFile("bigpapa.root");
	//TFile *f_reco_co   = new TFile("all_april30_8am.root");
	...
	//const int   trackbinbounds[trackbin]         = { 0,20,30,40,50,60,69,76,83};
	//const int   trackbinboundsUpper[trackbin]    = {20,30,40,50,60,69,76,83,1000};
	...
	int pt_lo 	= ptbinbounds_lo[0];
	int pt_hi 	= ptbinbounds_hi[0];
	...
	TH1D* hJ1_f1_reco_co;
	hJ1_f1_reco_co = (TH1D*)f_reco_co->Get("hJet_Pass")->Clone("j1_reco_co");

	double reco_co_Lx_1[trackbin] = {10,25,35,45,55,65,74,82,90};
	double reco_co_Ly_1[trackbin];
	double reco_co_Lxel_1[trackbin] = {1,1,1,1,1,1,1,1,1};
	double reco_co_Lxeh_1[trackbin] = {1,1,1,1,1,1,1,1,1};
	double reco_co_Lye_1[trackbin];

	double reco_co_Lx_2[trackbin] = {10,25,35,45,55,65,74,82,90};
	double reco_co_Ly_2[trackbin];
	double reco_co_Lxel_2[trackbin] = {1,1,1,1,1,1,1,1,1};
	double reco_co_Lxeh_2[trackbin] = {1,1,1,1,1,1,1,1,1};
	double reco_co_Lye_2[trackbin];

	double reco_co_Lx_3[trackbin] = {10,25,35,45,55,65,74,82,90};
	double reco_co_Ly_3[trackbin];
	double reco_co_Lxel_3[trackbin] = {1,1,1,1,1,1,1,1,1};
	double reco_co_Lxeh_3[trackbin] = {1,1,1,1,1,1,1,1,1};
	...
	for(int wtrk = 0; wtrk < trackbin; wtrk++){
		...
		TH2D* h1;
		h1 = (TH2D*)f_reco_co->Get(Form("hSigS_Cor_%d_to_%d_and_%d_to_%d_w_PU_1",trackbinbounds[wtrk], trackbinboundsUpper[wtrk], pt_lo, pt_hi))->Clone();
		h1->Scale(1/(hJ1_f1_reco_co->GetBinContent(special_trk+1)));
		h1->Scale(1./(BW1));

		TH2D* h2;
		h2 = (TH2D*)f_reco_co->Get(Form("hBckS_Cor_%d_to_%d_and_%d_to_%d_w_PU_1",trackbinbounds[wtrk], trackbinboundsUpper[wtrk], pt_lo, pt_hi))->Clone();
	...
	TH1D* hJ1_f2_data_co;
	hJ1_f2_data_co = (TH1D*)f_data_co->Get("hJet_Pass")->Clone("j2_data_co");

	double data_co_Lx_1[trackbin] = {10,25,35,45,55,65,74,82,90};
	double data_co_Ly_1[trackbin];
	double data_co_Lxel_1[trackbin] = {1,1,1,1,1,1,1,1,1};//{10,5,5,5,5,5,4,4,4};
	double data_co_Lxeh_1[trackbin] = {1,1,1,1,1,1,1,1,1};//{10,5,5,5,5,5,4,4,10};
	double data_co_Lye_1[trackbin];

	double data_co_Lx_2[trackbin] = {10,25,35,45,55,65,74,82,90};
	double data_co_Ly_2[trackbin];
	double data_co_Lxel_2[trackbin] = {1,1,1,1,1,1,1,1,1};//{10,5,5,5,5,5,4,4,4};
	double data_co_Lxeh_2[trackbin] = {1,1,1,1,1,1,1,1,1};//10,5,5,5,5,5,4,4,10};
	double data_co_Lye_2[trackbin];

	double data_co_Lx_3[trackbin] = {10,25,35,45,55,65,74,82,90};
	double data_co_Ly_3[trackbin];
	double data_co_Lxel_3[trackbin] = {1,1,1,1,1,1,1,1,1};//{10,5,5,5,5,5,4,4,4};;
	double data_co_Lxeh_3[trackbin] = {1,1,1,1,1,1,1,1,1};//{10,5,5,5,5,5,4,4,10};
	...
	for(int wtrk = 0; wtrk < trackbin; wtrk++){
                        ...
                        //if(wtrk <5 ) special_trk = wtrk;
                        //if(wtrk >=5 ) special_trk = wtrk+4;
                        //if(wtrk >=5 ) special_trk = wtrk+7;
		TH2D* h1;
		h1 = (TH2D*)f_data_co->Get(Form("hSigS_Cor_%d_to_%d_and_%d_to_%d_w_PU_1",trackbinbounds[wtrk], trackbinboundsUpper[wtrk], pt_lo, pt_hi))->Clone();
		h1->Scale(1./(hJ1_f2_data_co->GetBinContent(special_trk+1)));
		h1->Scale(1./(BW1));

		TH2D* h2;
		h2 = (TH2D*)f_data_co->Get(Form("hBckS_Cor_%d_to_%d_and_%d_to_%d_w_PU_1",trackbinbounds[wtrk], trackbinboundsUpper[wtrk], pt_lo, pt_hi))->Clone();
	...
	reco_co_grL1->SetLineColor(kRed);
	reco_co_grL1->SetMarkerColor(kRed);
	reco_co_grL2->SetLineColor(kRed);
	reco_co_grL2->SetMarkerColor(kRed);
	reco_co_grL3->SetLineColor(kRed);
	reco_co_grL3->SetMarkerColor(kRed);


	reco_co_mg1->Add(reco_co_grL1,"pc");
	reco_co_mg1->Add(reco_co_grL2,"pc");
	reco_co_mg1->Add(reco_co_grL3,"pc");
	reco_co_mg1->Add(data_co_grL1,"pc");
	reco_co_mg1->Add(data_co_grL2,"pc");
	reco_co_mg1->Add(data_co_grL3,"pc");
	...
	TFile* fS_tempA = new TFile("sys_jetpt_5_30_83.root", "recreate");
